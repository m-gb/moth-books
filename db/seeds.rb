...
Author.delete_all
Author.create id: 1, name: "J.R.R. Tolkien"
Author.create id: 2, name: "Orson Scott Card"
Author.create id: 3, name: "Stephen King"
...
CSV.foreach(Rails.root.join("db/seeds_data/books.csv"), headers: true) do |row|
  Book.find_or_create_by(id: row[0], title: row[1], author_id: row[2], price: row[3], category_id: row[4], year: row[5], image_filename: row[6], description: row[7])
end
